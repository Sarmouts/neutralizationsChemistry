...
int main()
{
	...
	while (true)
	{
		...
		else
		{
			if (x == v)
			{
				answer = "H" + to_string(x) + X + " + " + Y + "OH" + to_string(v) + " --> " + to_string(x) + "H2O + " + Y + X;
				...
				HO = v * x;
				answer = to_string(v) + "H" + to_string(x) + X + " + " + to_string(x) + Y + "OH" + to_string(v) + " --> " + to_string(HO) + "H2O + " + Y + to_string(x) + X + to_string(v);
				cout << replace(answer) << endl;
